...
def create_msg(vendor, wdir, slide_id, msg_input, msg_input_dict, msg_input_dict2, model_name, pred_label, pred_score):
    """
    ...
    if vendor == "3DHistech":
        ...
        tmp_slidedir = Path(rf"{wdir}/tmp_slides/{slide_no_ext}/{slide_no_ext}.mrxs")
    else:
        slide_no_ext = os.path.splitext(slide_id)[0]
        tmp_slidedir = Path(rf"{wdir}/tmp_slides/{slide_no_ext}/{slide_id}")

    tmp_resdir = Path(f"{wdir}/results_inference/{slide_no_ext}/{model_name}")
    ...
    path_to_mask = Path(f"{tmp_resdir}/masks/{slide_no_ext}.jpg")

    path_to_csv = Path(f"{tmp_resdir}/model-outputs-csv/{slide_no_ext}.csv")
